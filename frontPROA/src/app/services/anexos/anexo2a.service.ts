...
export class Anexo2aService {

  constructor(private datePipe: DatePipe, private maskcpf: ValidadorcpfcnpjService) { }

  async anexo2A (embarcacao: Embarcacao, solicitacao: string, campotexto1: string, campotexto2: string, campotexto3: string, servico?: string): Promise<void | Uint8Array>{

    try{

      const pdfBytes = await fetch('assets/pdfanexos/Anexo2A-N212.pdf').then(res => res.arrayBuffer());
      ...
      switch (solicitacao){
        ...
        case "mudancanome":
          ...
          form.getTextField('campotexto1').setText(campotexto1?.toUpperCase() ?? '');
          form.getTextField('campotexto2').setText(campotexto2?.toUpperCase() ?? '');
          form.getTextField('campotexto3').setText(campotexto3?.toUpperCase() ?? '');
        break;
