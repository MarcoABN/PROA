...
export class Anexo1CService {

  constructor(private maskcpf: ValidadorcpfcnpjService) {}


  async anexo1C(cliente?: Cliente, servico?: string): Promise<void | Uint8Array> {

    

    try {
      const pdfBytes = await fetch('assets/pdfanexos/Anexo1C-N212.pdf').then(res => res.arrayBuffer());
      ...
      if (cliente) {
        ...
        const endereco =
          ...
          ', ' + (cliente.cidade ?? '') +
          ', CEP: ' + (cliente.cep ?? '');
        const [part1, part2] = this.divideString(endereco, 85);
